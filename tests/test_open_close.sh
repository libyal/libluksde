...
{ 
	...
	${TEST_RUNNER} ./${LUKSDE_TEST_OPEN_CLOSE} ${INPUT_FILE};
...
{ 
	...
	then
		...
		${TEST_RUNNER} ./${LUKSDE_TEST_OPEN_CLOSE} -p${PASSWORD} ${INPUT_FILE};
...
LUKSDE_TEST_OPEN_CLOSE="luksde_test_open_close";

if ! test -x ${LUKSDE_TEST_OPEN_CLOSE};
then
	LUKSDE_TEST_OPEN_CLOSE="luksde_test_open_close.exe";
fi

if ! test -x ${LUKSDE_TEST_OPEN_CLOSE};
then
	echo "Missing executable: ${LUKSDE_TEST_OPEN_CLOSE}";
...
then
	...
	then
		IGNORE_LIST=`cat input/.libluksde/ignore | sed '/^#/d'`;
	fi
