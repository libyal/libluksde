...
#include "luksde_test_libcthreads.h"
#include "luksde_test_libluksde.h"
#include "luksde_test_unused.h"

/* Define to make luksde_test_read generate verbose output
#define LUKSDE_TEST_READ_VERBOSE
 */

#define LUKSDE_TEST_READ_BUFFER_SIZE		4096
#define LUKSDE_TEST_READ_NUMBER_OF_THREADS	4
...
/* Tests libluksde_volume_read_buffer
 * Returns 1 if successful, 0 if not or -1 on error
 */
int luksde_test_read_buffer(
     libluksde_volume_t *volume,
     size64_t input_size,
     size64_t expected_size )
{
	uint8_t buffer[ LUKSDE_TEST_READ_BUFFER_SIZE ];
	...
	{
		read_size = LUKSDE_TEST_READ_BUFFER_SIZE;
...
/* Tests libluksde_volume_read_buffer_at_offset
 * Returns 1 if successful, 0 if not or -1 on error
 */
int luksde_test_read_buffer_at_offset(
     libluksde_volume_t *volume,
     off64_t input_offset,
     size64_t input_size,
     off64_t expected_offset,
     size64_t expected_size )
{
	...
	if( libluksde_volume_get_offset(
	     volume,
	     &result_offset,
	     &error ) != 1 )
	{
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int luksde_test_seek_offset_and_read_buffer(
     libluksde_volume_t *volume,
     off64_t input_offset,
     int input_whence,
     size64_t input_size,
     ...
     size64_t expected_size )
{
	...
	if( result == 1 )
	{
		if( input_offset >= 0 )
		{
			result = luksde_test_read_buffer(
				  volume,
				  input_size,
				  expected_size );
		}
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int luksde_test_read_from_volume(
     libluksde_volume_t *volume,
     size64_t volume_size )
{
	 ...
	 */
	result = luksde_test_seek_offset_and_read_buffer(
	          volume,
	          0,
	          SEEK_SET,
	          volume_size,
	 ...
	 */
	result = luksde_test_read_buffer_at_offset(
	          volume,
	          (off64_t) ( volume_size / 7 ),
	          volume_size / 2,
 ...
 */
int luksde_test_read_callback_function(
     libluksde_volume_t *volume,
     void *arguments LUKSDE_TEST_ATTRIBUTE_UNUSED )
{
	uint8_t buffer[ LUKSDE_TEST_READ_BUFFER_SIZE ];

	libcerror_error_t *error = NULL;
	static char *function    = "luksde_test_read_callback_function";
	size_t read_size         = LUKSDE_TEST_READ_BUFFER_SIZE;
	ssize_t read_count       = 0;
	int number_of_iterations = 3;
	int result               = 0;

	LUKSDE_TEST_UNREFERENCED_PARAMETER( arguments )
 ...
 * Returns 1 if successful, 0 if not or -1 on error
 */
int luksde_test_read_from_volume_multi_thread(
     libluksde_volume_t *volume,
     size64_t volume_size,
     int number_of_threads )
{
	...
	static char *function                  = "luksde_test_read_from_volume_multi_thread";
	...
	if( number_of_threads > 1 )
	{
		...
		if( (size64_t) expected_offset > volume_size )
		{
			...
			number_of_iterations = volume_size / LUKSDE_TEST_READ_BUFFER_SIZE;

			if( ( volume_size % LUKSDE_TEST_READ_BUFFER_SIZE ) != 0 )
			{
		...
		if( libcthreads_thread_pool_create(
		     ...
		     number_of_threads,
		     number_of_iterations,
		     (int (*)(intptr_t *, void *)) &luksde_test_read_callback_function,
		     NULL,
		     &error ) != 1 )
		{
			libcerror_error_set(
...
#endif
{
...
#if defined( HAVE_DEBUG_OUTPUT ) && defined( LUKSDE_TEST_READ_VERBOSE )
	...
	if( luksde_test_read_from_volume(
	     volume,
	     volume_size ) != 1 )
	{
		fprintf(
...
#if defined( HAVE_MULTI_THREAD_SUPPORT )
	if( luksde_test_read_from_volume_multi_thread(
	     volume,
	     volume_size,
	     LUKSDE_TEST_READ_NUMBER_OF_THREADS ) != 1 )
	{
		fprintf(
		 stderr,
