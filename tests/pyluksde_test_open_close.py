...
def pyluksde_test_single_open_close_file(filename, mode, password=None):
  ...
  try:
    ...
    if (not filename and
        exception.message == "pyluksde_volume_open: unsupported string object type."):
      pass
    ...
    if (mode == "w" and
        exception.message == "pyluksde_volume_open: unsupported mode: w."):
      pass
...
def pyluksde_test_single_open_close_file_object(filename, mode, password=None):
...
def pyluksde_test_multi_open_close_file_object(filename, mode, password=None):
...
def main():
  ...
  if not pyluksde_test_single_open_close_file(
  ...
  if not pyluksde_test_multi_open_close_file(
      options.source, "r", password=options.password):
    return False

  if not pyluksde_test_single_open_close_file_object(
      options.source, "r", password=options.password):
    return False

  if not pyluksde_test_single_open_close_file_object_with_dereference(
      options.source, "r", password=options.password):
    return False

  if not pyluksde_test_multi_open_close_file_object(
      options.source, "r", password=options.password):
    return False
