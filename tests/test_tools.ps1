...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$Result = RunTestBinary ${TestExecutablesDirectory} "luksde_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$Result = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "luksdeinfo" ${TestProfile} "-u" ${TestInput}
