...
then
	docker run -t -v "${PWD}:/libluksde" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libluksde && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libluksde;
